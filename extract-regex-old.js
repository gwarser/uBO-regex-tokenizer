// before uBO 1.31.9 https://github.com/gorhill/uBlock/commit/0bbf5b52abfa58fbd0cb505063011b57409aada1
...
function extractTokenFromRegex(pattern) {
    ...
    let reRegexToken = /[%0-9A-Za-z]{2,}/g;
    let reRegexTokenAbort = /[([]/;
    let reRegexBadPrefix = /(^|[^\\]\.|[*?{}\\])$/;
    let reRegexBadSuffix = /^([^\\]\.|\\[dw]|[([{}?*.]|$)/;
    let maxTokenLen = 7;
    ...
    while ((matches = reRegexToken.exec(s)) !== null) {
        ...
        if (
            reRegexBadPrefix.test(prefix) || (
                ...
                reRegexBadSuffix.test(
                    s.slice(reRegexToken.lastIndex)
                )
            )
        ) {
            continue;
        ...
        if (badTokens.has(token) === false) { return token; }
    }
}
